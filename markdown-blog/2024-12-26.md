...
Open `Config.fs` and update the values in quotes to match your site. The most important one is `siteBaseUrl` — set it to your site's address (the one from step 2). This ensures your RSS feed and sitemap have correct links. See the [Customization](customization.html) article for details.
