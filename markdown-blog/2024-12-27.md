...
Open `Config.fs` in the root of your repository. You'll find six values at the top — just change the text between the quotation marks:
...
let siteLanguage = "en"
let siteAuthor = ""
let siteImage = "assets/avatar.jpg"
```
...
- **siteImage** — the preview image shown when your blog links are shared on social media. Defaults to your avatar; leave empty (`""`) to skip

Right below these values, the same file contains the interface texts — the "blog entries" heading, the "Published on" label, and the 404 page messages. If your blog is not in English, translate them there.

You don't need to know F# — just edit the text between the quotation marks and push. That's it.

## Avatar and icons

The avatar at the top can be found in the `/assets/` folder as `avatar.jpg`. You can replace it with another one. If you'd rather use a file with a different name or format (say, an animated GIF), drop it into `/assets/` and update the `<img>` tag in `header.html` in the `/html/` folder. Next to it are icons for various browsers and devices, such as favicon.svg. You can replace these files with others.

The avatar also serves as the preview image when someone shares a link to your blog on social media — that's the `siteImage` value from the configuration above. For best results keep it square and at least 200x200 pixels.
