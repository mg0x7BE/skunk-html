...
Dark mode works automatically — your site respects your visitors' browser/OS preference. No toggle is needed. This is handled by the `color-mode="user"` attribute on the `<html>` tag, which tells MVP.css to follow the system's `prefers-color-scheme` setting.
